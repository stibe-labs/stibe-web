...
function IndustryCard({ ind }: { ind: any }) {
  ...
  return (
    <div ref={ref} className="relative min-h-[110vh] w-full flex items-center justify-center px-4 sm:px-6">
      <div className="relative w-full max-w-5xl h-[65vh] sm:h-[75vh] flex flex-col items-center justify-end pb-10 sm:pb-20">
        ...
        <motion.div style={{ y: textY }} className="relative z-10 w-full px-6 sm:px-12 flex flex-col items-center text-center">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center shadow-2xl backdrop-blur-xl mb-6">
            <ind.icon size={36} className="text-white drop-shadow-md" />
          </div>
          <h3 className="text-3xl sm:text-4xl md:text-5xl font-black mb-4 sm:mb-6 text-white tracking-widest uppercase drop-shadow-2xl">
            {ind.title}
          </h3>
          <p className="text-neutral-400 text-base sm:text-xl leading-relaxed mb-8 sm:mb-10 font-medium max-w-2xl mx-auto drop-shadow-md">
            {ind.desc}
          </p>
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            {ind.tags.map((tag: string) => (
              <span key={tag} className="px-5 py-2.5 rounded-full bg-white/10 border border-white/10 text-white text-xs sm:text-sm font-bold tracking-widest uppercase backdrop-blur-md shadow-lg">
                {tag}
              </span>
            ))}
          </div>
